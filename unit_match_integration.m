...
%% User input: 
UMparam.SaveDir = 'Z:\ibn-vision\DATA\SUBJECTS\M23034\ephys\20230805\probe0\unitmatch'; % Recommended to use end this path with \Probe0\IMRO_1\ if more probes/IMRO tables were used or \AllProbes\AllIMRO\ otherwise
UMparam.KSDir = {'Z:\ibn-vision\DATA\SUBJECTS\M23034\ephys\20230805\probe0\sorters\kilosort3\sorter_output'};  % This is a cell array with a path, in the path there should be a subfolder called 'RawWaveforms'. 
...
UMparam.AllDecompPaths = {'Z:\ibn-vision\DATA\SUBJECTS\M23034\ephys\20230805\probe0_preprocessed'};  % This is a cell array with info on where to find the decompressed recording (.bin files) --> Necessary when you want UnitMatch to do waveform extraction
RecordingSites_Recording1 = readNPY("Z:\ibn-vision\DATA\SUBJECTS\M23034\ephys\20230805\probe0\sorters\kilosort3\sorter_output\channel_positions.npy");
...
%% Optional (for Kilosort + SpikeGLX users) --- see ExampleAnalysisPipelines for more detail!!
 UMparam = ExtractKilosortData(UMparam.KSDir, UMparam,UMparam.AllDecompPaths); % Extract KS data and do some noise removal, optionally decompresses cbin to bin data and uses BOMBCELL quality metric to define good single units
