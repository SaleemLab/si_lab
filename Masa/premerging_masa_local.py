...
for probe in range(int(no_probe)):
    ...
    if use_ks4:
        ...
        probe0_preprocessed_corrected = si.load_extractor(save_folder+'probe'+str(probe)+'_preprocessed')
        probe0_sorting_ks4 = si.run_sorter(sorter_name= 'kilosort4',recording=probe0_preprocessed_corrected,output_folder=save_folder+'probe'+str(probe)+'/sorters/kilosort4/',do_correction=False,use_binary_file=True)
        ...
        probe0_ks4_spikes = np.load(save_folder + 'probe'+str(probe)+'/waveform/kilosort4/sorting/spikes.npy') # testing waveform folder rather than sorters folder for reading spikes.npy file
        #probe0_ks4_spikes = np.load(save_folder + 'probe'+str(probe)+'/sorters/kilosort4/in_container_sorting/spikes.npy')

        if not os.path.exists(save_folder + 'probe'+str(probe)+'/sorters/kilosort4/in_container_sorting/'): #if missing in_container_sorting folder, create one just for saving spike.csv in it
            os.makedirs(save_folder + 'probe'+str(probe)+'/sorters/kilosort4/in_container_sorting/')
            shutil.copyfile(save_folder + 'probe'+str(probe)+'/waveform/kilosort4/sorting/spikes.npy', save_folder + 'probe'+str(probe)+'/sorters/kilosort4/in_container_sorting/spikes.npy')
