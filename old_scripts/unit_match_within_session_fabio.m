...
addpath(genpath('C:\Users\fabio.rodrigues\Documents\GitHub\si_lab'))
base_folder = 'Z:\ibn-vision\DATA\SUBJECTS\';
mouses = ['M24035';'M24036';'M23024';'M23020';'M23022';'M23019'];
SESSION = {['20240523';'20240524'];...
...
for iDate = 1:size(dates,1)
    ...
    firststim_folder = fullfile(ephys_folder,[mouse, '_',date,'_g0']);
    fprintf(['\nNow doing mouse ',mouse,' session ', date])

    if exist(fullfile(firststim_folder,[mouse, '_',date,'_g0_imec1']),"dir")==0
        probenos = 1;
    else
        probenos = 2;
    end
    
    for no_probe = 1:probenos        

         UMparam.KSDir = {fullfile(ephys_folder,'kilosort4',['probe',num2str(no_probe)-1],'sorters','kilosort4','sorter_output')};  % This is a cell array with a path, in the path there should be a subfolder called 'RawWaveforms'.
...
%         UMparam.KSDir = {fullfile('Z:\ibn-vision\DATA\SUBJECTS\M23028\ephys\20230703\kilosort_probe_',num2str(no_probe))};
        ...
        UMparam.AllRawPaths ={fullfile(ephys_folder,'kilosort4',['probe',num2str(no_probe)-1,'preprocessed'])};
        UMparam.AllDecompPaths= {fullfile(ephys_folder,'kilosort4',['probe',num2str(no_probe)-1,'preprocessed'])};  % This is a cell array with info on where to find the decompressed recording (.bin files) --> Necessary when you want UnitMatch to do waveform extraction

         UMparam.AllChannelPos = {readNPY(fullfile(ephys_folder,'kilosort4',['probe',num2str(no_probe)-1],'sorters\kilosort4\sorter_output\channel_positions.npy'))}; % These are coordinates of every recording channel on the probe (e.g. nRecordingChannels x 2)
%         UMparam.AllChannelPos = {readNPY(fullfile(ephys_folder,['kilosort_probe_',num2str(no_probe)-1],'channel_positions.npy'))};
        ...
        % Specify the file path
         ks3_sparsity_path = fullfile(ephys_folder,'kilosort4',['probe',num2str(no_probe)-1],'waveform\kilosort4\sparsity.json');
         ks3_waveform_path = fullfile(ephys_folder,'kilosort4',['probe',num2str(no_probe)-1],'waveform\kilosort4\waveforms\');
...
%         ExtractKilosortData(UMparam.KSDir, Params)
        ...
        original_id = unit_id;
        savepath = fullfile(ephys_folder,['probe',num2str(no_probe)-1,'um_merge_suggestion.mat']);
